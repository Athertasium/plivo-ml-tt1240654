...
**Signal.** The model reads 20 causal prosodic features from the audio strictly
...
turn, recovered from the energy gate rather than from annotations. Two
...
silence-timer baseline at all — to do so at an 800 ms delay the model would
need ≥94 of 100 turn-ends to fire while keeping firing long holds inside 5
turns, and an AUC near 0.65 cannot buy both at once. The residual errors are
...
a perfect ranker would score 100 ms against our 1290 ms, so nearly all the gap
